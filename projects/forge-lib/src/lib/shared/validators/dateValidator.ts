...
export function dateFormatValidator(): ValidatorFn {
  return (control: AbstractControl): { [key: string]: any } | null => {
    ...
    var forbidden = !validateDateFormat(control.value);
    return forbidden ? { 'invalidDateFormat': { value: control.value } } : null;
  };
}

export function validateDateFormat(dateFormat: any): boolean {
  var val = dateFormat as string;
  var format = /^([m]{1,3}|[d]{1,2}|[y]{4}){1}[^A-Za-z0-9]{1,2}([m]{1,3}|[d]{1,2}|[y]{4}){1}[^A-Za-z0-9]{1,2}([m]{1,3}|[d]{1,2}|[y]{4}){1}$/g;
  var required = ((val.includes('m') || val.includes('mm') || val.includes('mmm')) &&
